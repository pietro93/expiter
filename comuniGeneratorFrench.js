...
function getRelated(province) {
  ...
  return [pick(), pick(), pick(), pick()].filter(Boolean).map(name => {
    ...
    if (!p) return null;
    return { name: fr(p.Name), region: fr(p.Region), abbr: p.Abbreviation, url: '/fr/province/' + handle(fr(p.Name)) + '/' };
  }).filter(Boolean);
}
...
fetch('https://expiter.com/dataset.json', { method: 'Get' })
  ...
  .then(data => {
    ...
    for (let i = 0; i < 107; i++) {
      ...
      const provinceSlug = handle(frProvince);
      const dirName = 'fr/municipalites/province-de-' + provinceSlug;
      ...
      const towns = comuniKeys.map(key => {
        ...
        return {
          ...
          density: c.Density,
          altitude: c.Altitude,
          url: '/fr/municipalites/' + provinceSlug + '/' + handle(name) + '/'
        };
      });
      ...
      const largestTown = {
        name: largestName,
        url: '/fr/municipalites/' + provinceSlug + '/' + handle(largestName) + '/',
      ...
      const html = nunjucks.render('pages/comuni-list-fr.njk', {
        lang: 'fr',
        seoTitle: 'Communes dans la Province de ' + frProvince + ', ' + frRegion + ', Italie',
        seoDescription: 'Liste de toutes les ' + towns.length + ' communes dans la province de ' + frProvince + ', ' + frRegion + ', Italie.',
        seoKeywords: frProvince + ' communes, municipalités ' + frProvince + ', communes ' + frProvince,
        ...
        heroAlt: 'Communes dans la Province de ' + frProvince,
        pageTitle: 'Communes dans la Province de ' + frProvince,
        eyebrow: frRegion + ' · Communes',
        breadcrumbParent: { href: '/fr/province/' + provinceSlug + '/', label: 'Province de ' + frProvince },
        sidebar: pb.setSideBarFR(province),
        toc: null,
        tabRows: null,
        provinceName: frProvince,
        provinceUrl: '/fr/province/' + provinceSlug + '/',
        regionName: frRegion,
      ...
      if (!fs.existsSync('fr/municipalites')) fs.mkdirSync('fr/municipalites', { recursive: true });
      fs.writeFile(dirName + '.html', html, err => {
        if (err) throw err;
        console.log(province.Name + ' comuni list (FR) saved!');
      });
    }
  })
