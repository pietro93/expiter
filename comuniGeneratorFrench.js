...
import {fr, handle} from './js/pageBuilder.js'
...
fetch('https://expiter.com/dataset.json', {method:"Get"})
    ...
    .then(function (data) {
        ...
        for (let i = 0; i < 107; i++){
            ...
            var dirName = 'fr/municipalites/province-de-'+handle(fr(province.Name)).replace(/'/g, '-').replace(/\s+/g, '-').toLowerCase();
            var fileName = '.html';
    ...
    '<nav id="navbar"></nav>'+
    '<div class="hero" style="background-image:url(\'https://expiter.com/img/'+province.Abbreviation+'.webp\')" '+'title="Communes dans la province de '+province.Name+'"'+'>'+
    '</div><h1 class="title">Communes à </h1>'+
    '<section id="Liste des communes dans la province de '+fr(province.Name)+'">'+
    ...
    // Effectuez vos opérations JavaScript ici
    var nComuni=0;
    $("h1").text("Communes dans la province de "+fr(province.Name))
    ...
    for (let c in dataset[i].Comuni){
        ...
        list+="<tr>"+
        '<th><h2>'+'<a href="https://expiter.com/fr/municipalites/'+handle(fr(province.Name))+'/'+handle(comune.Name)+'/">'+
    ...
    $("#info").html(
    "Il y a <b>"+nComuni+" communes</b> dans la province de "+
    '<a href="https://expiter.com/fr/provinces/'+handle(fr(province.Name))+'/">'+fr(province.Name)+'</a>'+
    " en "+province.Region+". </br>"+
    "<b>"+'<a href="https://expiter.com/fr/communes/'+handle(fr(province.Name))+'/'+handle(dataset[i]["Comuni"][firstComune].Name)+'/">'+
    dataset[i]["Comuni"][firstComune]["Name"]+
    "</a></b> est la plus grande ville en population avec un total de "+dataset[i]["Comuni"][firstComune]["Population"]+" habitants.")
...
function generateSiteMap(dataset){
        let comuniSiteMap='<?xml version="1.0" encoding="UTF-8"?> '+'\n'+
              ...
              for (let i = 0; i < 107; i++){
                let province = dataset[i];
                let urlPath = 'fr/municipalites/province-de-'+handle(fr(province.Name)).replace(/'/g, '-').replace(/\s+/g, '-').toLowerCase();
        ...
        fs.writeFile("sitemap/comuni-sitemap-fr.xml", comuniSiteMap, function (err, file) {
          if (err) throw err;
          console.log("comuni-sitemap-fr.xml"+' Saved!');
      });
      }
