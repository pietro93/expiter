...
fetch('https://expiter.com/dataset.json', {method:"Get"})
    ...
    .then(function (data) {
        ...
        for (let i = 0; i < 107; i++){
            ...
            var dirName = 'de/gemeinden/provinz-'+handle(de(province.Name)).replace(/'/g, '-').replace(/\s+/g, '-').toLowerCase();
            var fileName = '.html';
    ...
    '</div><h1 class="title">Gemeinden in </h1>'+
    '<section id="Liste der Gemeinden in der Provinz von '+de(province.Name)+'">'+
    ...
    var nComuni=0;
    $("h1").text("Gemeinden in der Provinz von "+de(province.Name))
    ...
    for (let c in dataset[i].Comuni){
        ...
        list+="<tr>"+
        '<th><h2>'+'<a href="https://expiter.com/de/gemeinden/'+handle(de(province.Name))+'/'+handle(comune.Name)+'/">'+
    ...
    $("#info").html(
    "Es gibt <b>"+nComuni+" Gemeinden</b> in der Provinz von "+
    '<a href="https://expiter.com/de/provinz/'+handle(de(province.Name))+'/">'+de(province.Name)+'</a>'+
    " in "+province.Region+". </br>"+
    "<b>"+'<a href="https://expiter.com/de/gemeinden/'+handle(de(province.Name))+'/'+handle(dataset[i]["Comuni"][firstComune].Name)+'/">'+
    dataset[i]["Comuni"][firstComune]["Name"]+
    "</a></b> ist die größte Stadt nach Bevölkerungszahl mit insgesamt "+dataset[i]["Comuni"][firstComune]["Population"]+" Einwohnern.")
...
function generateSiteMap(dataset){
        ...
        fs.writeFile("sitemap/comuni-sitemap-de.xml", comuniSiteMap, function (err, file) {
          if (err) throw err;
          console.log("comuni-sitemap-de.xml"+' Saved!');
      });
      }
