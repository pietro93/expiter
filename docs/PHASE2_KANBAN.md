...
- [x] 6.1 EN province pages — slugs unchanged. Generator uses `.replace(/\s+/g,"-").replace("'","-").toLowerCase()` (no accent strip), matches sitemap e.g. `province/forlì-cesena/`. ✓
- [x] 6.2 EN CaS pages — same slug logic, matches sitemap. ✓
- [x] 6.3 IT province pages — slugs unchanged. `.replace(/'/g,'-').replace(/\s+/g,'-').toLowerCase()`, accents preserved. ✓
- [x] 6.4 FR/DE/ES province pages — FR/ES: slug(translated+NFD-strip) unchanged from pre-migration. DE: `monza-e-brianza` → `monza-und-brianza` **REGRESSION — confirmed stale file. Fixed: 301 redirect added to `.htaccess`, stale `de/provinz/monza-e-brianza.html` deleted.**
- [x] 6.5 Comuni listing pages — EN/IT/FR/ES: no regressions. DE: `provinz-monza-e-brianza` → `provinz-monza-und-brianza` **REGRESSION — Fixed: redirect added, stale `de/gemeinden/provinz-monza-e-brianza.html` deleted.** (IT also note: `it/comuni/massa-carrara/` skipped — no data file, known gap)
- [x] 6.6 Town pages — EN/IT/FR/ES: no regressions (EN/IT accents preserved, FR/ES sitemaps confirmed). DE: `de/gemeinden/monza-e-brianza/*.html` (55 pages) → `de/gemeinden/monza-und-brianza/*.html` **REGRESSION — Fixed: wildcard 301 redirect added. Old directory left in place (redirect intercepts before serving stale files).**
- [x] 6.7 Region pages — all correct. EN sitemap confirms `region/regions-of-italy/`. IT/FR/DE/ES region pages are new (no prior indexed URLs). Capital links use translated+handle slugs, which match actual province page slugs. ✓
- [x] 6.8 `.htaccess` — CaS per-province `.htaccess` files unmodified. 3 DE Monza 301 redirects added to root `.htaccess`.
