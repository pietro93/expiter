...
function populateData(data){
  ...
  function dynamicSort(property) {
    ...
    function sortData(selection){
      ...
      let newHref="https://expiter.com/fr/app/"+
      "?sort="+sortParams[0]+
...
function appendData(data) {
    ...
    if (selection.length===0) {
      ...
      $("a.province1st").attr("href","https://expiter.com/fr/province/"+province1st.Name.replace(/\s/g,"-")
       .replace("'","-").toLowerCase()+"/")
      if (selection.length>1){ let province2=selection[1]
      $("a.province2").text(province2.Name)
      $("a.province2").attr("href","https://expiter.com/fr/province/"+province2.Name.replace(/\s/g,"-")
       .replace("'","-").toLowerCase()+"/")}
       if (selection.length>2){ let province3=selection[2]
      $("a.province3").text(province3.Name)
      $("a.province3").attr("href","https://expiter.com/fr/province/"+province3.Name.replace(/\s/g,"-")
       .replace("'","-").toLowerCase()+"/")}
      }
...
function searchParams(){
  ...
  function linkTo(province){
    return '<b><a href="https://expiter.com/fr/province/'+province.Name.replace("'","").replace(/\s+/g,"-").toLowerCase()+
    '"/>'+province.Name+'</a></b>'
  }
