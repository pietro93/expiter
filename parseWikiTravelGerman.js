...
for (var i=100; i<107; i++){
    ...
    const file = fs.createWriteStream("temp/de-parsedDataAbout"+province+".txt");

    let wikiUrl = de(province);
    wikiUrl = "https://de.m.wikivoyage.org/wiki/"+wikiUrl.replace(/\s/g,"_");
...
function parseData(province){
    const html = fs.readFileSync('temp/de-parsedDataAbout'+province+'.txt','utf8');
    console.log("parsing data about "+province+"...")
    const dom = new jsdom.JSDOM(html);
    const $ = require('jquery')(dom.window);
    format($,"#mf-section-0");
    $("#Hintergrund").parent().next().attr("id","infotarget1");
    ...
    let getAround=$("#Mobilität").parent().next().attr("id","GAtarget")
    format($,"#GAtarget")
    ...
    console.log('Writing temp/de-parsedDataAbout'+province+'.txt...')
    //facts[province]["parsedData"]=provinceData
    let parsedData = provinceData+"%%%"+getAround;
    fs.writeFile('temp/de-parsedDataAbout'+province+'.txt', parsedData, function (err, file) {
