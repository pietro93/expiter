...
for (var i=0; i<107; i++){
    ...
    const file = fs.createWriteStream("temp/es-parsedDataAbout"+province+".txt");

    let wikiUrl = es(province);
    wikiUrl = "https://es.m.wikivoyage.org/wiki/"+wikiUrl.replace(/\s/g,"_");
...
function parseData(province){
    const html = fs.readFileSync('temp/es-parsedDataAbout'+province+'.txt','utf8');
    console.log("parsing data about "+province+"...")
    const dom = new jsdom.JSDOM(html);
    const $ = require('jquery')(dom.window);
    format($,"#mf-section-0");
    $("#Comprender").parent().next().attr("id","infotarget1");
    $("#Geographie").parent().next().attr("id","infotarget2");
    ...
    let getAround=$("#Desplazarse").parent().next().attr("id","GAtarget")
    format($,"#GAtarget")
    ...
    console.log('Writing temp/es-parsedDataAbout'+province+'.txt...')
    //facts[province]["parsedData"]=provinceData
    let parsedData = provinceData+"%%%"+getAround;
    fs.writeFile('temp/es-parsedDataAbout'+province+'.txt', parsedData, function (err, file) {
